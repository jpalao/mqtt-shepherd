...
var Q = require('q'),
    ...
    mqtt = require('mqtt'),
    mosca = require('mosca'),
    debug = require('debug')('mqtt-shepherd:init');
...
init._setupAuthPolicy = function (shepherd) {
    ...
    broker.authenticate = function (client, user, pass, cb) {
        var defaultAccount = shepherd.defaultAccount,
            isAdded = shepherd._setClient(client),
            authorized = false;

        function feedback(err, isAuthorized) {
            if (!isAdded)
                cb(new Error('Client already exists.'), false);
            else
                cb(err, isAuthorized);
        }
...
init._attachBrokerEventListeners = function (shepherd) {
    ...
    broker.on('clientDisconnected', function (client) {
        var qnode = shepherd._nodebox[client.id],
           mClient = shepherd._getClient(client.id);
...
init._testShepherdPubSub = function (shepherd) {
    var deferred = Q.defer(),
        mc = shepherd.mClient,
        shepherdId = shepherd.clientId,
        testTopics = [ 'register', 'deregister', 'schedule', 'update', 'notify', 'response', 'ping', 'request', 'announce', 'lwt' ],
...
init._dispatchMessage = function (shepherd, intf, cId, topic, message) {
    var msgStr,
        ...
        objMsgChs = [ 'register', 'deregister', 'schedule', 'notify', 'update', 'response', 'ping', 'lwt' ];
    ...
    switch (intf) {
        ...
        case 'notify':
            ...
            _.forEach(parsedMsg, function (val, key) {
                if (key === 'oid')
                    parsedMsg.oid = mutils.oidKey(val) || val;
            });
    ...
    setImmediate(function () {
        if (unknownIntf)
            messageHandler(shepherd, topic, message, null);
...
init._attachShepherdMessageHandler = function (shepherd) {
    ...
    mc.on('message', function (topic, message, packet) {
        ...
        shepherd.decrypt(message, cId, function (err, decrypted) {
            if (err)
                debug('Decrytion fails. From client id: ' + cId);   // log 'decrytion fails'
            else
