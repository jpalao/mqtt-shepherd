...
MqttNode.prototype._dumpSummary = function () {
    var self = this,
        dumped = {},
        includedKeys = [ 'clientId', 'joinTime', 'lifetime', 'ip', 'mac', 'version', 'objList' ];
...
MqttNode.prototype.writeReq = function (path, data, callback) {
    ...
    return Q.fcall(function () {
        ...
        if (mutils.isGoodResponse(rsp.status)) {
            if (_.isNil(rsp.data))
                self.readReq(path).done();
            else
                self._checkAndUpdate(path, rsp.data).done();
        }
...
MqttNode.prototype.observeReq = function (path, opt, callback) {
    ...
    return Q.fcall(() => {
        var chkErr = _.isString(path) ? _noShepherdOrSoError(this) : new TypeError('path should be a string.');
        if (chkErr)
            throw chkErr;
        return this.shepherd.observeReq.apply(this.shepherd, mutils.turnPathToReqArgs(path, this.clientId, opt));
    }).then((rsp) => {
        return { status: rsp.status };
    }).fail((err) => {
        return makeTimeoutResponse(this, err);
    }).nodeify(callback);
};
